...
f <- paste("./FlatNet/plots/projector/", dataset, "/total_work.pdf", sep = "")
...
total_and_cdf.plot <- grid.arrange(arrangeGrob(total_work.plot, work_cdf.plot, nrow = 1))
...
throughput_and_makespan.plot <- grid.arrange(arrangeGrob(makespan.plot, throughput.plot, nrow = 1))
...
clusters.plot <- clusters.plot + 
  ...
  scale_fill_manual(values = c(ftng_color, fn_color, sn_color, dsn_color)) +
  coord_cartesian(xlim = c(0, 25)) +
  scale_y_sqrt(breaks = c(0, 1000, 5000, 10000, 20000, 40000),labels = function(x){paste0(x/1000)})
#breaks = seq(0, 16000, 1000), 
