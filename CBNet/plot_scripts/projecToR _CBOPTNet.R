...
total_work.table$abb <- factor(total_work.table$abb, levels = c("OPT", "CBN", "DSN", "CBOPT", "BT"))

total_work.table["operation"] <- revalue(total_work.table$operation, c("rotation" = "Rotation",
                                                                        "routing" = "Routing"))

total_work.table %>% filter(
  size %in% c(128)) -> total_work.table
...
ggsave(filename = "./plots/projecToR/total_work_7.png", units = "cm",
